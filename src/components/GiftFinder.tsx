...
export default function GiftFinder() {
  ...
  return (
    <div className="bg-white border border-brand-gold/10 rounded p-6 sm:p-8 md:p-10">
      {!showResults ? (
        <form onSubmit={handleSearch} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              <label className="block font-sans text-[10px] font-bold tracking-wider text-brand-gold uppercase">
                01. Who are you gifting?
              </label>
              <div className="relative">
                <select
                  value={recipient}
                  onChange={(e) => setRecipient(e.target.value)}
                  className="w-full bg-brand-ivory border border-brand-gold/25 rounded px-3.5 py-3 font-sans text-xs font-semibold uppercase tracking-wider text-brand-charcoal/80 focus:border-brand-rose focus:outline-none appearance-none cursor-pointer"
                  required
                >
          ...
          <div className="pt-4 border-t border-brand-gold/10 flex justify-center">
            <button
              type="submit"
              className="flex items-center justify-center rounded bg-brand-charcoal px-8 py-3.5 font-sans text-xs font-bold tracking-widest uppercase text-white hover:bg-brand-rose transition-colors duration-200 shadow-sm"
            >
        ...
        <div className="space-y-6 animate-fade-in">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-brand-gold/15 pb-4 gap-4">
            <div>
              <h3 className="font-serif text-xl font-light text-brand-charcoal">
                Curated Recommendations
              </h3>
              <p className="text-[10px] text-brand-charcoal/50 mt-1 uppercase tracking-wider font-sans font-bold">
          ...
          {recommended.length === 0 ? (
            <div className="text-center py-8">
              <Gift className="mx-auto h-8 w-8 text-brand-gold/50 mb-3 stroke-[1.5]" />
              ...
              <p className="text-xs text-brand-charcoal/50 mt-2 max-w-lg mx-auto leading-relaxed">
                Connect with our design concierges via WhatsApp. We build custom hampers tailored to your specific budget and recipient.
              </p>
              <div className="mt-6 flex justify-center">
                <Link
                  href="/customize"
                  className="rounded bg-brand-rose px-6 py-2.5 font-sans text-xs font-semibold tracking-wider uppercase text-white hover:bg-brand-rose/90 transition-all shadow-sm"
                >
            ...
            <div>
              ...
              <p className="text-center text-xs text-brand-charcoal/50 mt-6 font-sans">
